...
public class EmployeeDetailRouteController extends BaseRouteController {
	...
	public ModelAndView start(@RequestParam Map<String, String> object, HttpServletRequest request){
		...
		try{
			...
			if(ViewModelNames.IS_ELEVATED_USER.getValue())
			{
				modelAndView = new ModelAndView(ViewNames.EMPLOYEE_DETAIL.getViewName());
					...
					if (!activeUserEntity.isPresent()) {
						modelAndView.addObject(ViewModelNames.ERROR_MESSAGE.getValue(), "No active user."); 
						modelAndView = new ModelAndView(ViewNames.SIGN_IN.getViewName());
					} else {
						modelAndView.addObject(ViewModelNames.ERROR_MESSAGE.getValue(), "Error.");
